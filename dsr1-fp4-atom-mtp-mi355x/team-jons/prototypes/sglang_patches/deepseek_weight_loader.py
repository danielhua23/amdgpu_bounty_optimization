        ...
        for layer_id in layer_ids:
            ...
            if w.dtype in (
                ...
                if weight_block_size is not None:
                    ...
                    # PATCH: handle MTP head shape mismatch at TP > 1.
                    if weight_scale.dim() == 2 and weight.dim() == 2:
                        if weight_scale.shape[1] != weight.shape[1]:
                            if weight_scale.shape[0] == weight.shape[0] and weight_scale.shape[1] == 1:
                                pass
                            elif weight_scale.numel() == weight.shape[0]:
                                weight_scale = weight_scale.view(-1, 1)
                            elif weight_scale.shape[0] == weight.shape[1]:
                                weight_scale = weight_scale.t()
                    elif weight_scale.dim() == 1 and weight_scale.numel() == weight.shape[0]:
                        weight_scale = weight_scale.view(-1, 1)

                    w, scale = channel_quant_to_tensor_quant(weight, weight_scale)
                    self_attn.w_scale = scale
                    # PATCH: quark_post_load_weights only handles bf16/uint8 — cast fp8 back
                    if w.dtype == torch.float8_e4m3fn or w.dtype == torch.float8_e4m3fnuz:
