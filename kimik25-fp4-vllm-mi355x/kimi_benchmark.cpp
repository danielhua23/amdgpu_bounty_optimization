// ============================================
// Benchmark Script - kimik2.5 fp4 Multi-Concurrency Mode (C++ Version)
// ============================================
// Usage:
...
//   ./kimi_benchmark acc -isl 8192 -osl 1024       # Test specific CONC
//   ./kimi_benchmark submit <team> -isl 8192 -osl 1024   # Batch test CONC=4,32,128 + submit (only supported case)
...
// ============================================
// Baseline Data Structure (thresholds: max E2E ms, min interactivity, min tput per GPU)
// ============================================
struct Baseline {
    double median_e2e;   // max allowed E2E latency (ms)
    double median_intvty; // min required interactivity (token/s/user)
    double tput_per_gpu;  // min required throughput per GPU (token/s/GPU)
};

// Only ISL=8192, OSL=1024 (8k/1k); CONC = 4, 32, 128. Values are thresholds to meet.
map<string, Baseline> BASELINES = {
    {"8192_1024_4",   {6000,  150, 1350}},  // e2e ≤ 6s,   interactivity ≥ 150, tput ≥ 1350
    {"8192_1024_32",  {14000, 65,  4500}},  // e2e ≤ 14s,  interactivity ≥ 65,  tput ≥ 4500
    {"8192_1024_128", {24500, 35,  5300}},  // e2e ≤ 24.5s, interactivity ≥ 35,  tput ≥ 5300
};
...
string get_leaderboard_url(const string& isl, const string& osl, int conc) {
    ...
    if (conc == 4) return "https://daniehua-kimik25-fp4-isl8192-osl1024-conc4.hf.space";
    if (conc == 32) return "https://daniehua-kimik25-fp4-isl8192-osl1024-conc32.hf.space";
    if (conc == 128) return "https://daniehua-kimik25-fp4-isl8192-osl1024-conc128.hf.space";
...
int validate_accuracy(const AccuracyMetrics& metrics) {
    ...
    const double BASELINE_GSM8K_METRIC = stod(get_env_var("GSM8K_BASELINE_METRIC", "0.9325"));
...
# Baseline thresholds (min throughput, min interactivity, max e2e ms) - ISL=8192, OSL=1024
BASELINES = {
    ('8192', '1024', '4'):   {'median_e2e': 6000,  'median_intvty': 150, 'tput_per_gpu': 1350},
    ('8192', '1024', '32'):  {'median_e2e': 14000, 'median_intvty': 65,  'tput_per_gpu': 4500},
    ('8192', '1024', '128'): {'median_e2e': 24500, 'median_intvty': 35,  'tput_per_gpu': 5300},
}
...
try:
    ...
    # Output for leaderboard submit (no ratio fields): e2e, tput, baseline_e2e, baseline_tput, interactivity, baseline_interactivity, gsm8k
